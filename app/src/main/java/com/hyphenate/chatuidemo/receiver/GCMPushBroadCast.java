...
public class GCMPushBroadCast extends BroadcastReceiver
{
	...
	public void sendNotification(Context context, String message, boolean isForeground)
	{
		...
		try
		{
			...
			NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(context).setSmallIcon(context.getApplicationInfo().icon).setSound(defaultSoundUrlUri).setWhen(System.currentTimeMillis()).setAutoCancel(true);

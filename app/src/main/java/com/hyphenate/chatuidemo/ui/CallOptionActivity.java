...
public class CallOptionActivity extends BaseActivity implements View.OnClickListener
{
	...
	void initCameraResolutionSpinner(final int cameraId, final int spinnerId)
	{
		...
		try
		{
			...
			// update selection
			int selection = 0;
			String resolution = cameraId == Camera.CameraInfo.CAMERA_FACING_BACK ? PreferenceManager.getInstance().getCallBackCameraResolution() : PreferenceManager.getInstance().getCallFrontCameraResolution();

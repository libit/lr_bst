 ...
 */
public abstract class MyBaseBannerFragment extends MyBaseFragment
{
	private static final String TAG = MyBaseBannerFragment.class.getSimpleName();
	...
	protected void setViewPagerAdapter(List<BannerInfo> bannerInfoList)
	{
		if (bannerInfoList != null && bannerInfoList.size() > 0)
		{
			...
			{
				mFragmentBannerList.add(FragmentServerImage.newInstance(ApiConfig.getServerPicUrl(bannerInfo.getPicUrl()), DisplayTools.getWindowWidth(this.getContext()), bannerInfo.getContent()));
			}
			bannerSectionsPagerAdapter = new SectionsPagerAdapter(getChildFragmentManager(), mFragmentBannerList);

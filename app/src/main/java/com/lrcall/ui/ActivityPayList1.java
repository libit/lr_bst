...
public class ActivityPayList1 extends MyBaseActivity implements PayAdapter.IPayAdapterItemClicked
{
	public static final int REQ_PAY = 1002;
	private static final String TAG = ActivityPayList1.class.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		if (bundle != null)
		{
			type = bundle.getString(ConstValues.DATA_PAY_CONTENT);
			if (type.equals(ConstValues.PAY_ORDER) || type.equals(ConstValues.PAY_DATA_TRAFFIC_ORDER))
			{
		...
		OrderService mOrderService = new OrderService(this);
		mOrderService.addDataResponse(new IAjaxDataResponse()
		{
			@Override
			public boolean onAjaxDataResponse(String url, String result, AjaxStatus status)
			{
					...
					public boolean onAjaxDataResponse(String url, String result, AjaxStatus status)
					{
						if (url.endsWith(ApiConfig.GET_ALIPAY_CONIFG))
						{
							...
							if (alipayConfigInfo != null)
							{
								...
								alipayService.pay(ActivityPayList1.this, new Handler()
								{
									...
									public void handleMessage(Message msg)
									{
										switch (msg.what)
										{
											case AlipayService.SDK_PAY_FLAG:
											{
												...
												{
													Toast.makeText(ActivityPayList1.this, "支付成功", Toast.LENGTH_SHORT).show();
													finish();
												}
												else
												{
													...
													{
														Toast.makeText(ActivityPayList1.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
													}
													else
													{
														// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
														Toast.makeText(ActivityPayList1.this, "支付失败", Toast.LENGTH_SHORT).show();
													}
												}
												break;
											}
								...
								}, alipayConfigInfo, subject, orderId,String.format("%.2f", (double) orderInfo.getTotalPrice() / 100));
							}
							return true;
						}
						return false;
					...
					public boolean onAjaxDataResponse(String url, String result, AjaxStatus status)
					{
						if (url.endsWith(ApiConfig.WX_PRE_PAY))
						{
							...
							if (wxPayInfo != null)
							{
								...
								Toast.makeText(ActivityPayList1.this, "正常调起支付", Toast.LENGTH_SHORT).show();
								// 在支付之前，如果应用没有注册到微信，应该先调用IWXMsg.registerApp将应用注册到微信
								boolean res = api.sendReq(req);
								if (res)
								{
									Toast.makeText(ActivityPayList1.this, "请求支付成功！", Toast.LENGTH_SHORT).show();
								}
								else
								{
									ToastView.showCenterToast(ActivityPayList1.this, R.drawable.ic_do_fail, "请求支付失败！");
								}
								finish();
							}
							else
							{
								ToastView.showCenterToast(ActivityPayList1.this, R.drawable.ic_do_fail, "服务器返回信息不正确！");
							}
							return true;
						}
						else if (url.endsWith(ApiConfig.GET_WX_PAY_INFO))
						{
							...
							if (ReturnInfo.isSuccess(returnInfo))
							{
								...
								if (api == null)
								{
									api = WXAPIFactory.createWXAPI(ActivityPayList1.this, WxPayService.getWeixinAppId());

...
public abstract class AbstractAQuery<T extends AbstractAQuery<T>> implements Constants
{
	 ...
	 * @see testImage5
	 */
	public T image(String url, boolean memCache, boolean fileCache, int round)
	{
		return image(url, memCache, fileCache, 0, 0, null, 0, 0, round, null);
	}
	...
	{
		callback.targetWidth(targetWidth).fallback(resId).url(url).memCache(memCache).fileCache(fileCache);
	...
	public File getCachedFile(String url)
	{
		...
		if (result == null)
			result = AQUtility.getExistedCacheByUrl(AQUtility.getCacheDir(getContext(), AQuery.CACHE_DEFAULT), url);
		return result;
	}
	 ...
	 * <p/>
	 * This method is less efficient and promote file sync which could hold up the UI thread.
	 * <p/>
